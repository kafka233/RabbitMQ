...
class RabbitMQ(object):
    ...
    def connection(self):
        connectParams = pika.ConnectionParameters(host=self.ip,credentials=self.credentials,virtual_host=self.virtualHost)
        try:
    ...
    def producer(self,msg,connection):
        ...
        channel.basic_publish(exchange='',routing_key=self.queuesName,body=msg,properties=pika.BasicProperties(delivery_mode=2))
